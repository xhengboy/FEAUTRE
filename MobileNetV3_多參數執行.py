...
if __name__ == "__main__":
    ...
    # 定義要搜尋的參數組合
    param_grid = list(itertools.product([200,300,400,500], [16,32], [0.001]))

    #param_grid = list(itertools.product([1, 2], [32, 64], [1, 0.1]))
    print("共有 {} 組參數組合".format(len(param_grid)))

    # 外層迴圈：依序嘗試不同組參數
    for epochs, batch_size, lr in param_grid:
        print("="*50)
        print(f"本次訓練參數：EPOCHS={epochs}, BATCH_SIZE={batch_size}, LEARNING_RATE={lr}")
        # 更新 Config 參數與輸出路徑（避免覆寫前次結果）
        Config.EPOCHS = epochs
        Config.BATCH_SIZE = batch_size
        Config.LEARNING_RATE = lr
        Config.OUTPUT_DIR = os.path.join(Config.BASE_DIR, "640_SIZE_MobileNetV3","new","Single_fusion",
                                          f"Epochs_{epochs}_BatchSize_{batch_size}_LR_{lr}")
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
            print(f"參數組 {epochs}_{batch_size}_{lr} - Fold {fold + 1}/{Config.K_FOLDS}")
            ...
            for epoch in range(Config.EPOCHS):
                ...
                with tqdm(train_loader, desc=f"Epoch {epoch+1}/{Config.EPOCHS} [Train]") as t:
                    for images, labels_batch in t:
                        ...
                        t.set_postfix({'Loss': f"{train_loss/(t.n+1):.4f}",
                ...
                print(f"Epoch {epoch+1}: Train {epoch_train_loss:.4f}/{epoch_train_acc:.2f}% | Test {epoch_test_loss:.4f}/{epoch_test_acc:.2f}%")
            ...
            if train_accuracies and test_accuracies:
                ...
                plt.legend()
                plt.grid()
                acc_plot_path = os.path.join(Config.OUTPUT_DIR, f'fold_{fold+1}_accuracy_curve.jpg')
                plt.savefig(acc_plot_path, dpi=1200)
                plt.close()
                ...
                plt.xlabel('Predicted')
                plt.ylabel('True')
                cm_path1 = os.path.join(Config.OUTPUT_DIR, f'fold_{fold + 1}_confusion_matrix.jpg')
                plt.savefig(cm_path1, dpi=1200)
                plt.close()
                # 保存模型
                model_save_path = os.path.join(Config.OUTPUT_DIR, f'fold_{fold + 1}_model.h5')
                torch.save(model.state_dict(), model_save_path)
            # 將每折結果寫入 CSV 檔案
            metrics_csv_path = os.path.join(Config.Accuracy, f'metrics_{lr}.csv')
            with open(metrics_csv_path, 'a', newline='') as f:
                writer = csv.writer(f)
                if is_first_fold:
                    writer.writerow([])
                    writer.writerow(['EPOCHS', Config.EPOCHS, 'BATCH_SIZE', Config.BATCH_SIZE, 'LEARNING_RATE', Config.LEARNING_RATE])
                    writer.writerow(['Fold', 'F1 Score', 'Precision', 'Recall', 'Train Loss', 'Train Acc', 'Test Loss', 'Test Acc'])
                    is_first_fold = False
        ...
        # 儲存該組參數全部交叉驗證結果的統計數據到 CSV
        final_metrics_csv_path = os.path.join(Config.Accuracy, f'final_metrics_{lr}.csv')
        ...
        print(f"參數組 Epochs={epochs}, BatchSize={batch_size}, LR={lr} 完成交叉驗證，記憶體已釋放。\n")
